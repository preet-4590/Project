...
$total_students = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) AS total FROM students WHERE institution = '$inst' AND is_deleted = 0")
)['total'];

$in_today = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) AS total FROM student_attendance WHERE institution = '$inst' AND direction = 'IN' AND entry_type = 'student' AND DATE(log_time) = '$today'")
)['total'];

$out_today = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) AS total FROM student_attendance WHERE institution = '$inst' AND direction = 'OUT' AND entry_type = 'student' AND DATE(log_time) = '$today'")
)['total'];

$active_guests = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) AS total FROM guest_passes WHERE is_active = 1 AND '$today' BETWEEN valid_from AND valid_until AND institution = '$inst'")
)['total'];

$guest_passes_result = mysqli_query($conn,
    "SELECT * FROM guest_passes WHERE institution = '$inst' ORDER BY issued_at DESC LIMIT 5"
);

$search = mysqli_real_escape_string($conn, $_GET['search'] ?? '');
$student_query = "SELECT * FROM students WHERE institution = '$inst' AND is_deleted = 0";
if (!empty($search)) {
    $student_query .= " AND (name LIKE '%$search%' OR roll_no LIKE '%$search%' OR unique_id LIKE '%$search%')";
}
$student_query .= " ORDER BY unique_id DESC";
...
<head>
    ...
    <title><?php echo htmlspecialchars($inst); ?> Dashboard | CGEMS</title>
    <style>
        :root {
            --navy:    #0f172a;
            --blue:    #1e3d8f;
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .nav-item .badge.purple { background: var(--purple); }
        ...
        .flash {
            ...
            font-weight: 500; margin-bottom: 24px; display: flex;
            align-items: center; gap: 10px;
            transition: opacity 0.6s;
        }
        .flash-green  { background:#f0fdf4; color:#166534; border:1px solid #bbf7d0; }
        ...
        .section-header a {
            font-size: 13px; color: var(--blue2); font-weight: 600;
        ...
        .search-bar .btn-clear:hover { background: #e2e8f0; }
...
<aside class="sidebar">
    ...
    <div class="sidebar-inst">
        ...
        <span><?php echo htmlspecialchars($inst); ?></span>
    </div>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="issue_guest_pass.php" class="nav-item">
            ...
            <?php if ($active_guests > 0): ?>
                <span class="badge purple"><?php echo $active_guests; ?></span>
            <?php endif; ?>
        </a>
    ...
    <div class="sidebar-bottom">
        <div class="sidebar-user">
            <div class="user-avatar"><?php echo strtoupper(substr($_SESSION['clerk_user'], 0, 1)); ?></div>
...
<div class="main">
    ...
    <header class="topbar">
        <div class="topbar-left">
            <h2>Dashboard — <?php echo htmlspecialchars($inst); ?></h2>
    ...
    <div class="content">
        ...
        $msg = $_GET['msg'] ?? '';
        $flash_map = [
            'deleted' => ['flash-red',   '🗑️ Student record removed from portal. Data retained in database.'],
            'restored'=> ['flash-green',  '♻️ Student record restored successfully.'],
            'updated' => ['flash-blue',   '✅ Student details updated successfully.'],
        ];
        if (isset($flash_map[$msg])):
            [$cls, $txt] = $flash_map[$msg];
        ?>
        <div id="flash-msg" class="flash <?php echo $cls; ?>">
            <?php echo $txt; ?>
        </div>
        <script>
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                ...
                <div class="stat-body">
                    <h4>Active Guests</h4>
                    <span><?php echo $active_guests; ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="search-bar">
            <form method="GET" style="display:flex;gap:10px;width:100%;">
                <input type="text" name="search" placeholder="Search by name, roll number or unique ID..."
                       value="<?php echo htmlspecialchars($search); ?>">
                <button type="submit">Search</button>
                <?php if (!empty($search)): ?>
                    <button type="button" class="btn-clear" onclick="window.location='dashboard.php'">Clear</button>
                <?php endif; ?>
            </form>
        </div>
                ...
                ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <small style="color:var(--muted);"><?php echo htmlspecialchars($row['gender'] ?? ''); ?></small>
                        </td>
                        ...
                        <td>
                            ...
                            <a href="delete_student.php?id=<?php echo $enc; ?>" class="btn-action btn-del"
                               onclick="return confirm('Remove this student from the portal?')">🗑️ Delete</a>
                        </td>
                    </tr>
                <?php endwhile; else: ?>
        ...
        <div class="section-header">
            <h3>🪪 Recent Guest Passes
                <?php if ($active_guests > 0): ?>
                    <span style="background:var(--purple);color:white;font-size:11px;padding:3px 10px;border-radius:20px;font-weight:600;">
            ...
            <table>
                ...
                ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($gp['institution']); ?></td>
